...
def get_value(filename) -> dict:
    ...
    data_dict['Возмещаемые затраты'] = total_cost * 1,2
...
def answer(filename1, filename2=None):
    ...
    if filename2 is None:
        ...
        # TODO: убрать () у значений
        if dict2['Возмещаемые затраты'] != dict1['Возмещаемые затраты']:
            answer += f'Возмещаемые затраты\n\nВ предварительном расчете размер возмещаемых затраты составляет: {dict1["Возмещаемые затраты"]} руб.\n'\
                    f'В расчете по факту: {dict2["Возмещаемые затраты"]} руб.\n'
            answer += 'Возмещаемые затраты больше -> Инвест. затраты (основной долг клиента) погашаются медленнее -> Начислено больше процентов за период лизинга' if \
                    dict2['Возмещаемые затраты'] > dict1['Возмещаемые затраты'] else 'Возмещаемые затраты меньше -> Инвест. затраты (основной долг клиента) '\
                        'погашаются быстрее -> Начислено меньше процентов за период лизинга'

        # Описание изменения аванса
        if dict2['Размер аванса'] != dict1['Размер аванса']:
            answer += f'Аванс\n\nВ предварительном расчете размер аванса составляет: {dict1["Размер аванса"]} руб.\n'\
                    f'В расчете по факту: {dict2["Размер аванса"]} руб.\n'
            if dict2['Размер аванса'] > dict1['Размер аванса']:
                answer += 'Аванс больше -> Инвест. затраты (основной долг клиента) погашаются быстрее -> Начислено меньше процентов за период лизинга'
            else:
                answer += 'Аванс меньше -> Инвест. затраты (основной долг клиента) погашаются медленнее -> Начислено больше процентов за период лизинга'
        ...
        if dict2['Отсрочка ДЛ'] != dict1['Отсрочка ДЛ']:
            ...
            if dict2['Отсрочка ДЛ'] > dict1['Отсрочка ДЛ']:
                ...
                answer += 'Отсрочка меньше -> Инвест. затраты (основной долг клиента) погашаются быстрее -> Начислено меньше процентов за период лизинга'
